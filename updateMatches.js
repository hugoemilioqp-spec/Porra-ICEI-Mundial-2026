...
(async function main() {
  try {
    ...
    // ========== 5. AVANCE AUTOMÁTICO DE GANADORES (CON BANDERAS) ==========
    ...
    const getFullTeamName = (match, cleanWinner) => {
      if (!cleanWinner) return null;
      const homeClean = cleanName(match.home);
      const awayClean = cleanName(match.away);
      if (homeClean === cleanName(cleanWinner)) return match.home;
      if (awayClean === cleanName(cleanWinner)) return match.away;
      return cleanWinner;
    };
    ...
    for (const m of koMatches) {
      ...
      if (m.homeScore > m.awayScore) {
        ...
        winner = getFullTeamName(m, m.winnerTeam);
      }
      if (!winner) continue;
